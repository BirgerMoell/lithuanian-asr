...
def get_args():
    ...
    parser.add_argument(
        "--data-prep-dir",
        default="./out/data/psst-fairseq-combined-augmented",
        ...
        with open(manifest_filename, "w") as mf, open(transcript_filename, "w") as tf:
            ...
            for utterance in split_data:
                if utterance.duration_frames > MAX_TOKENS:
                    ...
                    continue
                # augmentation on PSST

                base_dir = "/home/bmoell/psst-data/psst-data-2022-03-02/"

                full_path = base_dir + split_name + "/" + utterance.filename

                mf.write(f"{full_path}\t{utterance.duration_frames}\n")
                tf.write(f"{utterance.transcript}\n")

                augment_path, duration_frames = augment_gaussian(full_path)

                mf.write(f"{augment_path}\t{duration_frames}\n")
                tf.write(f"{utterance.transcript}\n")

                augment_path, duration_frames = augment_pitch_shift(full_path)
